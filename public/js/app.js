...
function getStatMeta(label) {
  ...
  if (value.includes('aws')) return { icon: 'fa-brands fa-aws', badge: 'CLOUD PLATFORM' };
  if (value.includes('ci') || value.includes('cd')) return { icon: 'fa-solid fa-code-branch', badge: 'PIPELINE AUTO' };
  if (value.includes('container') || value.includes('docker') || value.includes('kubernetes')) return { icon: 'fa-solid fa-cubes', badge: 'ORCHESTRATION' };
...
function getSkillStyle(item = '') {
  ...
  if (key.includes('aws') || key.includes('ec2') || key.includes('s3') || key.includes('iam') || key.includes('vpc')) return { icon: 'fa-brands fa-aws', color: '#FF9900' };
  if (key.includes('github') || key.includes('git')) return { icon: 'fa-brands fa-github', color: '#F5F5F5' };
  if (key.includes('docker') || key.includes('container')) return { icon: 'fa-brands fa-docker', color: '#2496ED' };
  if (key.includes('kubernetes') || key.includes('pod') || key.includes('deploy') || key.includes('service') || key.includes('secret') || key.includes('pv') || key.includes('pvc')) return { icon: 'fa-solid fa-dharmachakra', color: '#326CE5' };
  if (key.includes('terraform') || key.includes('ansible') || key.includes('iac')) return { icon: 'fa-solid fa-server', color: '#7B42BC' };
  if (key.includes('prometheus') || key.includes('grafana') || key.includes('cloudwatch')) return { icon: 'fa-solid fa-chart-line', color: '#E6522C' };
  if (key.includes('sonar') || key.includes('security') || key.includes('iam')) return { icon: 'fa-solid fa-shield-alt', color: '#4E9A51' };
  if (key.includes('python') || key.includes('bash') || key.includes('javascript')) return { icon: 'fa-brands fa-python', color: '#3776AB' };
  if (key.includes('linux') || key.includes('ubuntu')) return { icon: 'fa-brands fa-linux', color: '#FCC624' };
  if (key.includes('copilot') || key.includes('chatgpt') || key.includes('k8sgpt') || key.includes('ai')) return { icon: 'fa-solid fa-robot', color: '#7C3AED' };
  if (key.includes('jenkins') || key.includes('ci') || key.includes('cd')) return { icon: 'fa-solid fa-code-branch', color: '#D24939' };
...
function applySettings(settings) {
  ...
  if (meta) meta.content = settings.siteDescription || '';
